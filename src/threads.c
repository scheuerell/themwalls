...
static int fault_condition_exists = 0;
...
int process(jack_nframes_t nframes, void *arg)
{
	...
	// mjs if recording write new data to the disk buffer
	// mjs else write new data to the circular buffer
	if (rec) {
	    pos = disk_write_pos;
	    for (i = 0; i < nframes; i++) {
    ...
    // mjs if recording update disk buffer index
	// mjs else update circular buffer index
    if (rec) {
...
int writer_thread(void *d)
{
  ...
  again:
    ...
    parts = localtime(&t);
    if (safe_filename) {
	filename = g_strdup_printf("/media/pi/USB_DISK/%s%04d-%02d-%02d_%02d-%02d-%02d.%s",
	     prefix,
	...
	// mjs added 
	if (usb_removable_prep(dev_path) != 0) {
		while (usb_removable_prep(dev_path) != 0) {
			fault_condition_exists = 1;
			usleep(1000000);
		}
		fault_condition_exists = 0;
		recording_stop();
		goto again;
	}
    ...
    /* mjs this shouldnt be cleared because it will contain good data in the event of back-to-back triggers
    ...
    */
	...
	// mjs commented out sf_writef_float
    ...
    //mjs added
    /* unmount the drive */	
	if (usb_removable_unmount(dev_path) != 0) {
		printf("Unable to unmount removable usb storage device.");
...
int blink_thread(void *d)
{
	...
	while (!quiting) {
		usleep(100000);
		if (recording) {
			// continue to blink and save audio until PB is let go but at least 1 second
			while (i < 5 || digitalRead (PB)) {
				digitalWrite (LED, 0) ;     
				usleep(100000);           
				digitalWrite (LED, 1) ;     
				usleep(100000);
		...
		// stop here for pushbotton holdoff effect (debounce)
		recording_stop();
		i = 0;
		}
		// blink fast if a fault condition exits
		//while (!recording) {
		while (fault_condition_exists) {
			digitalWrite (LED, 0) ;     
			usleep(200000);           
			digitalWrite (LED, 1) ;     
			usleep(20000);
...
void process_init(unsigned int time)
{
    ...
    if (time < 1) {
	fprintf(stderr, "walltalk: buffer time must be 1 second or "
			"greater\n");
	exit(1);
    }
    if (time > MAX_TIME) {
	fprintf(stderr, "walltalk: buffer time must be %d seconds or "
