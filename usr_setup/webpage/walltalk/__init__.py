...
class walltalkTextbox(object):

    def __init__(self, text, element, walltalk):
        self._walltalk = walltalk
        self._id = 'textbox_' + str(int(random.uniform(0, 1e16)))
        self._walltalk._handlers.enqueue(
...
class walltalkInput(object):

    def __init__(self, input_type, walltalk, placeholder):
        self._walltalk = walltalk
    ...
    def get_text(self):
        text = self._walltalk._handlers.enqueue_and_result({'cmd': 'getinput', 'eid': self._id})
        return text

class walltalkListItem(object):

    def __init__(self, walltalk, parent_id, item_text, chevron, toggle, onclick, ontoggle):
        self._walltalk = walltalk
        ...
        if toggle:
          tg_flag = '1'
        self._walltalk._handlers.enqueue({'cmd': 'addli', 'eid': self._id, 'pid': self._parent_id,
            'txt': item_text, 'chevron': ch_flag, 'toggle': tg_flag, 'tid': self._toggle_id})

class walltalkList(object):
    """A walltalk page element representing an HTML <ul>."""
    def __init__(self, walltalk, page):
        self._walltalk = walltalk
        self._page = page
        self._id = 'ul_' + str(int(random.uniform(0, 1e16)))
        self._walltalk._handlers.enqueue({'cmd': 'addul', 'eid': self._id})

    def add_item(self, item_text, chevron=False, toggle=False, onclick=None, ontoggle=None):
        item = walltalkListItem(self._walltalk, self._id, item_text,
                            chevron, toggle, onclick, ontoggle)
        self._page._clickables[item._id] = item
        self._page._toggleables[item._toggle_id] = item
        return item

class walltalkButton(object):

    def __init__(self, text, walltalk, on_click):
        self._walltalk = walltalk
        self._id = 'button_' + str(int(random.uniform(0, 1e16)))
        self._walltalk._handlers.enqueue({'cmd': 'addbutton', 'eid': self._id, 'txt': text})
        self._on_click = on_click

    def set_text(self, text):
        self._walltalk._handlers.enqueue({'cmd': 'updateinner', 'eid': self._id, 'txt': text})


class walltalkImage(object):

    def __init__(self, src, walltalk):
        self._walltalk = walltalk
    ...
    def set_src(self, src):
        self._walltalk._handlers.enqueue({'cmd': 'setimagesrc', 'eid': self._id, 'src': src})
...
class walltalkPage(object):

    def __init__(self, walltalk, title, prev_text, onprevclick):
        self._walltalk = walltalk
        self._title = title
    ...
    def postPush(self):
        ...
        self._walltalk._handlers.enqueue(msg)

    def print_line(self, line):
        self._walltalk._handlers.enqueue({"cmd": "print", "msg": line})

    def add_textbox(self, text, element="p"):
        txtbox = walltalkTextbox(text, element, self._walltalk)
        self._elements.append(txtbox)
        return txtbox

    def add_element(self, element):
        ele = walltalkTextbox("", element, self._walltalk)
        self._elements.append(ele)
        return ele

    def add_button(self, text, on_click):
        button = walltalkButton(text, self._walltalk, on_click)
        self._elements.append(button)
        self._clickables[button._id] = button
        return button

    def add_input(self, input_type, placeholder=""):
        edit = walltalkInput(input_type, self._walltalk, placeholder)
        self._elements.append(edit)
        self._inputs[edit._id] = edit
        return edit

    def add_image(self, src):
        img = walltalkImage(src, self._walltalk)
        self._elements.append(img)
        return img

    def add_list(self):
        new_list = walltalkList(self._walltalk, self)
...
class walltalk(object):
    ...
    def console(self, title='', prev_text=None, onprevclick=None):
        page = walltalkPage(self, title, prev_text, onprevclick)
